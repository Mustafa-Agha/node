...
func Test_Expire_1_new(t *testing.T) {
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99999.9990e8), GetAvail(ctx, addr0, "CE"))
	...
	assert.Equal(int64(99999.9990e8), GetAvail(ctx, addr1, "CE"))
...
func Test_Expire_2a_new(t *testing.T) {
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99899.9990e8), GetAvail(ctx, addr0, "CE"))
...
func Test_Expire_2aa_new(t *testing.T) {
	...
	oidB := GetOrderId(addr0, 5, ctx)
	msgB := order.NewNewOrderMsg(addr0, oidB, 1, "BTC-000_CE", 1e8, 10e8)
	_, err := testClient.DeliverTxSync(msgB, testApp.Codec)
	assert.NoError(err)

	oidS := GetOrderId(addr1, 0, ctx)
	msgS := order.NewNewOrderMsg(addr1, oidS, 2, "BTC-000_CE", 1e8, 10e8)
	...
	*/
	...
	assert.Equal(int64(99945.9776e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(10e8), GetLocked(ctx, addr0, "CE"))
	assert.Equal(int64(99990e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(100043.9780e8), GetAvail(ctx, addr1, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
}
...
func Test_Expire_2b_new(t *testing.T) {
	...
	msgS := order.NewNewOrderMsg(addr1, oidS, 2, "BTC-000_CE", 11e8, 10e8)
	_, err := testClient.DeliverTxSync(msgS, testApp.Codec)
	assert.NoError(err)
...
func Test_Expire_2bb_new(t *testing.T) {
	...
	*/
	...
	assert.Equal(int64(100010e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99969.9850e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr0, "CE"))
...
func Test_Expire_3_new(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		oid := GetOrderId(addr0, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_CE", int64(i+2)*1e8, int64(i+1)*1e8)
		_, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
		if i == 2 {
			for j := 0; j < 2; j++ {
	...
	for i := 0; i < 3; i++ {
		oid := GetOrderId(addr1, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr1, oid, 2, "BTC-000_CE", int64(i+5)*1e8, int64(i+1)*1e8)
		_, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
		if i == 2 {
			for j := 0; j < 2; j++ {
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99956e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(44e8), GetLocked(ctx, addr0, "CE"))
	...
	msgB5 := order.NewNewOrderMsg(addr0, oidB5, 1, "BTC-000_CE", 1e8, 10e8)
	_, err := testClient.DeliverTxSync(msgB5, testApp.Codec)
	assert.NoError(err)

	oidB6 := GetOrderId(addr0, 6, ctx)
	msgB6 := order.NewNewOrderMsg(addr0, oidB6, 1, "BTC-000_CE", 4.25e8, 10e8)
	_, err = testClient.DeliverTxSync(msgB6, testApp.Codec)
	assert.NoError(err)

	oidS5 := GetOrderId(addr1, 5, ctx)
	msgS5 := order.NewNewOrderMsg(addr1, oidS5, 2, "BTC-000_CE", 4.75e8, 10e8)
	_, err = testClient.DeliverTxSync(msgS5, testApp.Codec)
	assert.NoError(err)

	oidS6 := GetOrderId(addr1, 6, ctx)
	msgS6 := order.NewNewOrderMsg(addr1, oidS6, 2, "BTC-000_CE", 10e8, 10e8)
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99947.4990e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(52.5e8), GetLocked(ctx, addr0, "CE"))
	assert.Equal(int64(99980e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(99999.9990e8), GetAvail(ctx, addr1, "CE"))
...
func Test_Expire_4a_new(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		oid := GetOrderId(addr0, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr0, oid, 1, "BTC-000_CE", 1e8, 10e8)
		_, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
	}

	oidS := GetOrderId(addr1, 0, ctx)
	msgS := order.NewNewOrderMsg(addr1, oidS, 2, "BTC-000_CE", 1e8, 15e8)
	_, err := testClient.DeliverTxSync(msgS, testApp.Codec)
	assert.NoError(err)
...
func Test_Expire_4b_new(t *testing.T) {
	...
	oidB := GetOrderId(addr0, 0, ctx)
	msgB := order.NewNewOrderMsg(addr0, oidB, 1, "BTC-000_CE", 1e8, 15e8)
	_, err := testClient.DeliverTxSync(msgB, testApp.Codec)
	assert.NoError(err)

	for i := 0; i < 3; i++ {
		oid := GetOrderId(addr1, int64(i), ctx)
		msg := order.NewNewOrderMsg(addr1, oid, 2, "BTC-000_CE", 1e8, 10e8)
		_, err := testClient.DeliverTxSync(msg, testApp.Codec)
		assert.NoError(err)
	}
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99985e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(15e8), GetLocked(ctx, addr0, "CE"))
...
func Test_Expire_5a_new(t *testing.T) {
	...
	msgB1 := order.NewNewOrderMsg(addr0, oidB1, 1, "BTC-000_CE", 1e8, 1e8)
	msgB1.TimeInForce = 3
	_, err := testClient.DeliverTxSync(msgB1, testApp.Codec)
	assert.NoError(err)

	oidS := GetOrderId(addr1, 0, ctx)
	msgS := order.NewNewOrderMsg(addr1, oidS, 2, "BTC-000_CE", 2e8, 2e8)
	msgS.TimeInForce = 3
	_, err = testClient.DeliverTxSync(msgS, testApp.Codec)
	assert.NoError(err)
	...
	assert.Equal(int64(100000e8), GetAvail(ctx, addr0, "BTC-000"))
	assert.Equal(int64(99999.9999e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr0, "CE"))
	assert.Equal(int64(100000e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(99999.9999e8), GetAvail(ctx, addr1, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr1, "BTC-000"))
}
...
func Test_Expire_5b_new(t *testing.T) {
	...
	*/
	...
	msgB2 := order.NewNewOrderMsg(addr0, oidB2, 1, "BTC-000_CE", 2e8, 1e8)
	msgB2.TimeInForce = 3
	...
	assert.Equal(int64(99997.9989e8), GetAvail(ctx, addr0, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr0, "CE"))
...
/*
test #6: expire fee is larger than the ce balance, the ce balance becomes 0
*/
...
/*
test #7: no ce balance, expire fee is charged in the balance of the opposite token
without ce trade, expire fee of the opposite token is calculated using ce init price
*/
func Test_Expire_7_new(t *testing.T) {
	...
	assert.Equal(int64(99985e8), GetAvail(ctx, addr1, "BTC-000"))
	assert.Equal(int64(0), GetAvail(ctx, addr1, "CE"))
...
/*
test #7: small ce balance, expire fee is charged in the balance of the opposite token
without ce trade, expire fee of the opposite token is calculated using ce init price
transfers order for the same user: CE -> BTC -> ETH
*/
func Test_Expire_8_new(t *testing.T) {
	...
	msgS3 := order.NewNewOrderMsg(addr2, oidS3, 2, "BTC-000_CE", 9e8, 15e8)
	...
	assert.Equal(int64(0.0002e8), GetAvail(ctx, addr2, "CE"))
	...
	assert.Equal(3, len(transfers))
	assert.Equal("CE", transfers[0].Fee.Tokens[0].Denom)
	...
	assert.Equal(int64(0e8), GetAvail(ctx, addr2, "CE"))
}

/*
test #7: no ce balance, expire fee is charged in the balance of the opposite token
with ce trade, expire fee of the opposite token is calculated using latest ce price
*/
func Test_Expire_9_new(t *testing.T) {
	...
	oidB2 := GetOrderId(addr2, 0, ctx)
	msgB2 := order.NewNewOrderMsg(addr2, oidB2, 1, "BTC-000_CE", 9e8, 1e8)
	_, err = testClient.DeliverTxSync(msgB2, testApp.Codec)
	assert.NoError(err)
	...
	msgS2 := order.NewNewOrderMsg(addr3, oidS2, 2, "BTC-000_CE", 9e8, 1e8)
	_, err = testClient.DeliverTxSync(msgS2, testApp.Codec)
	assert.NoError(err)
	...
	// since there is a trade, BTC-000_CE price is no longer 10, but 9
	// btc expire fee: 0.001 ce, 0.001/9 = 0.00011111 btc
	...
	assert.Equal(int64(99990.9955e8), GetAvail(ctx, addr2, "CE"))
	assert.Equal(int64(0), GetLocked(ctx, addr2, "CE"))
	assert.Equal(int64(99999e8), GetAvail(ctx, addr3, "BTC-000"))
	assert.Equal(int64(100008.9955e8), GetAvail(ctx, addr3, "CE"))
