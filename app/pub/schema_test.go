...
func TestExecutionResultsMarshaling(t *testing.T) {
	...
	trades := trades{
		NumOfMsgs: 1,
		Trades: []*Trade{{
			Id: "42-0", Symbol: "NNB_TNT", Price: 100, Qty: 100,
			Sid: "s-1", Bid: "b-1", TickType: 1,
			Sfee: "TNT:8;ETH:1", Bfee: "TNT:10;BTC:1", SSingleFee: "TNT:8;ETH:1", BSingleFee: "TNT:10;BTC:1",
			SAddr: "s", BAddr: "b", SSrc: 0, BSrc: 0}},
	}
	orders := Orders{
		NumOfMsgs: 3,
		Orders: []*Order{
			{"NNB_TNT", orderPkg.Ack, "b-1", "", "b", orderPkg.Side.BUY, orderPkg.OrderType.LIMIT, 100, 100, 0, 0, 0, "", 100, 100, orderPkg.TimeInForce.GTE, orderPkg.NEW, "", ""},
			{"NNB_TNT", orderPkg.FullyFill, "b-1", "42-0", "b", orderPkg.Side.BUY, orderPkg.OrderType.LIMIT, 100, 100, 100, 100, 100, "TNT:10;BTC:1", 100, 100, orderPkg.TimeInForce.GTE, orderPkg.NEW, "", "TNT:10;BTC:1"},
			{"NNB_TNT", orderPkg.FullyFill, "s-1", "42-0", "s", orderPkg.Side.SELL, orderPkg.OrderType.LIMIT, 100, 100, 100, 100, 100, "TNT:8;ETH:1", 99, 99, orderPkg.TimeInForce.GTE, orderPkg.NEW, "", "TNT:8;ETH:1"},
		},
	}
	proposals := Proposals{
		NumOfMsgs: 3,
	...
	delAddr, _ := sdk.AccAddressFromBech32("tnt1e2y8w2rz957lahwy0y5h3w53sm8d78qex2jpan")
...
func TestBooksMarshaling(t *testing.T) {
	publisher := NewKafkaMarketDataPublisher(Logger, "", false)
	book := OrderBookDelta{"NNB_TNT", []PriceLevel{{100, 100}}, []PriceLevel{{100, 100}}}
...
func TestAccountsMarshaling(t *testing.T) {
	publisher := NewKafkaMarketDataPublisher(Logger, "", false)
	accs := []Account{{"b-1", "TNT:1000;BTC:10", 0, []*AssetBalance{{Asset: "TNT", Free: 100}}}}
...
func TestBlockFeeMarshaling(t *testing.T) {
	publisher := NewKafkaMarketDataPublisher(Logger, "", false)
	msg := BlockFee{1, "TNT:1000;BTC:10", []string{"bnc1", "bnc2", "bnc3"}}
...
func TestTransferMarshaling(t *testing.T) {
	publisher := NewKafkaMarketDataPublisher(Logger, "", false)
	msg := Transfers{42, 20, 1000, []Transfer{{TxHash: "123456ABCDE", Memo: "1234", From: "", To: []Receiver{{"bnc1", []Coin{{"TNT", 100}, {"BTC", 100}}}, {"bnc2", []Coin{{"TNT", 200}, {"BTC", 200}}}}}}}
...
func TestCrossTransferMarsha(t *testing.T) {
	...
	msg := CrossTransfers{
		...
		Transfers: []CrossTransfer{
			{TxHash: "xxxx", ChainId: "rialto", Type: "xx", From: "xxxx", RelayerFee: 1, Denom: "TNT", To: []CrossReceiver{{Addr: "xxxx", Amount: 100}}},
			{TxHash: "xxxx", ChainId: "rialto", Type: "xx", From: "xxxx", RelayerFee: 0, Denom: "TNT", To: []CrossReceiver{{Addr: "xxxx", Amount: 100}}},
		},
	}
...
func TestStakingMarshaling(t *testing.T) {
	...
	msg := StakingMsg{
		...
		Delegations:       dels,
		DelegateEvents:    map[string][]*DelegateEvent{"chain-id-1": {&DelegateEvent{delAddr, valAddr, Coin{Denom: "TNT", Amount: 99999999}, "0xadkjgege"}}},
...
func TestSlashMarshaling(t *testing.T) {
	publisher := NewKafkaMarketDataPublisher(Logger, "", false)
	valAddr, _ := sdk.ValAddressFromBech32("bva1e2y8w2rz957lahwy0y5h3w53sm8d78qexkn3rh")
	submitterAddr, _ := sdk.AccAddressFromBech32("tnt1e2y8w2rz957lahwy0y5h3w53sm8d78qex2jpan")
