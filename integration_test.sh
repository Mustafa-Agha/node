...
function prepare_node() {
	...
	secret=$(./cechaind init --moniker testnode --home ${home} --home-client ${cli_home} --chain-id ${chain_id} | grep secret | grep -o ":.*" | grep -o "\".*"  | sed "s/\"//g")
	...
	# stop and start node
	ps -ef  | grep cechaind | grep testnoded | awk '{print $2}' | xargs kill -9
...
alice_addr=$(./cecli keys list --home ${cli_home} | grep alice | grep -o "ce1[0-9a-zA-Z]*")
bob_addr=$(./cecli keys list --home ${cli_home} | grep bob | grep -o "ce1[0-9a-zA-Z]*")
...
# send
result=$(expect ./send.exp ${cli_home} alice ${chain_id} "100000000000000:CE" ${bob_addr})
check_operation "Send Token" "${result}" "${chain_operation_words}"

sleep 1s
# multi send
echo ${bob_addr}
result=$(expect ./multi_send.exp ${cli_home} alice ${chain_id} "[{\"to\":\"${bob_addr}\",\"amount\":\"100000000000000:CE\"},{\"to\":\"${alice_addr}\",\"amount\":\"100000000000000:CE\"}]")
...
result=$(expect ./propose_list.exp ${chain_id} alice 200000000000:CE ${lower_case_btc_symbol} ce 100000000 "list BTC/CE" "list BTC/CE" ${cli_home} ${expire_time} 5)
...
sleep 3s
# list trading pair
result=$(expect ./list.exp ${btc_symbol} CE 100000000 bob ${chain_id} ${cli_home} 1)
check_operation "List Trading Pair" "${result}" "${chain_operation_words}"

sleep 1s
# place buy order
result=$(expect ./order.exp ${btc_symbol}_CE 1 100000000 1000000000 alice ${chain_id} gte ${cli_home})
...
sleep 2s
# cancel order
result=$(expect ./cancel.exp "${btc_symbol}_CE" "${order_id}" alice ${chain_id} ${cli_home})
...
echo ""
./cecli dex show -l ${btc_symbol}_CE  --trust-node true

sleep 1s
# place Sell order
result=$(expect ./order.exp ${btc_symbol}_CE 2 100000000 2000000000 bob ${chain_id} gte ${cli_home})
check_operation "Place Order" "${result}" "${chain_operation_words}"

result=$(./cecli dex show -l ${btc_symbol}_CE  --trust-node true)
...
sleep 1s
# place buy order
result=$(expect ./order.exp ${btc_symbol}_CE 1 100000000 2000000000 alice ${chain_id} gte ${cli_home})
check_operation "Place Order" "${result}" "${chain_operation_words}"
...
result=$(./cecli account $bob_addr --trust-node)
...
sleep 1s
result=$(expect ./send.exp ${cli_home} alice ${chain_id} "100000000000000:CE" ${bob_addr} "123456abcd")
check_operation "Send Token" "${result}" "${chain_operation_words}"
...
sleep 1s
result=$(expect ./send.exp ${cli_home} alice ${chain_id} "100000000000000:CE" ${bob_addr} "1234567890")
check_operation "Send Token" "${result}" "${chain_operation_words}"
...
result=$(expect ./HTLT-cross-chain.exp 2000 "100000000:CE" "100000000:CE" $bob_addr 0xf2fbB6C41271064613D6f44C7EE9A6c471Ec9B25 alice ${chain_id} ${cli_home})
...
atimicSwap=$(./cecli token query-swap --swap-id ${swapID} --trust-node)
...
result=$(./cecli account ce1wxeplyw7x8aahy93w96yhwm7xcq3ke4f8ge93u --trust-node)
...
# Create an atomic swap
result=$(expect ./HTLT-cross-chain.exp 2000 "100000000:CE" "100000000:CE" $alice_addr 0xf2fbB6C41271064613D6f44C7EE9A6c471Ec9B25 bob ${chain_id} ${cli_home})
check_operation "Create an atomic swap" "${result}" "${chain_operation_words}"
swapID=$(sed 's/swapID: //g' <<< $(echo "${result}" | tail -n 1 | grep -o "swapID: [0-9a-z]*"))
...
result=$(expect ./HTLT-single-chain.exp 2000 "100000000:CE" "10000:${eth_symbol}" $bob_addr alice ${chain_id} ${cli_home})
...
sleep 1s
# Create a single chain atomic swa
result=$(expect ./HTLT-single-chain.exp 360 "100000000:CE" "10000:${eth_symbol}" $bob_addr alice ${chain_id} ${cli_home})
check_operation "Create a single chain atomic swap" "${result}" "${chain_operation_words}"
randomNumber=$(sed 's/Random number: //g' <<< $(echo "${result}" | grep -o "Random number: [0-9a-z]*"))
timestamp=$(sed 's/Timestamp: //g' <<< $(echo "${result}" | grep -o "Timestamp: [0-9]*"))
randomNumberHash=$(sed 's/Random number hash: //g' <<< $(echo "${result}" | grep -o "Random number hash: [0-9a-z]*"))
...
sleep 3s
# list trading pair
result=$(expect ./list_mini.exp ${mbc_symbol} CE 100000000 alice ${chain_id} ${cli_home})
check_operation "List Trading Pair" "${result}" "${chain_operation_words}"

sleep 1s
# place buy order
result=$(expect ./order.exp ${mbc_symbol}_CE 1 100000000 1000000000 bob ${chain_id} gte ${cli_home})
check_operation "Place Order" "${result}" "${chain_operation_words}"
order_id=$(echo "${result}" | tail -n 1 | grep -o "[0-9A-Z]\{4,\}-[0-9]*") # capture order id, not symbol
printf "Order ID: $order_id\n"
...
sleep 2s
# cancel order
result=$(expect ./cancel.exp "${mbc_symbol}_CE" "${order_id}" bob ${chain_id} ${cli_home})
check_operation "Cancel Order" "${result}" "${chain_operation_words}"
...
echo ""
./cecli dex show -l ${mbc_symbol}_CE  --trust-node true

sleep 1s
# place Sell order
result=$(expect ./order.exp ${mbc_symbol}_CE 2 100000000 2000000000 alice ${chain_id} gte ${cli_home})
check_operation "Place Order" "${result}" "${chain_operation_words}"

result=$(./cecli dex show -l ${mbc_symbol}_CE  --trust-node true)
check_operation "Order Book" "${result}" "${order_book_words}"
